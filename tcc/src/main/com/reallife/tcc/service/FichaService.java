...
public class FichaService {
    ...
    public FichaDto toDto(FichaDeTreino ficha) {
        ...
        dto.setId(ficha.getId()); // ADICIONE ESTA LINHA
    ...
    // ========== MÉTODOS PARA O ALUNO VER TREINOS ==========
    public FichaDto verTreinoAtivoAluno(Long alunoId) {
        if (!alunoRepository.existsById(alunoId)) {
            throw new RuntimeException("Aluno não encontrado");
        }

        FichaDeTreino ficha = fichaRepository.findFichaAtivaByAlunoId(alunoId)
                .orElseThrow(() -> new RuntimeException("Nenhum treino ativo encontrado"));

        return toDto(ficha);
    }

    public List<FichaDto> verTodosTreinosAluno(Long alunoId) {
        if (!alunoRepository.existsById(alunoId)) {
            throw new RuntimeException("Aluno não encontrado");
        }

        List<FichaDeTreino> fichas = fichaRepository.findAllByAlunoIdOrderByDataCriacaoDesc(alunoId);
